...
public class CashierVerticle extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) {
    ...
    CashierStatsRepository statsRepo = new CashierStatsRepositoryImpl(chaosPool);
    CashierStatByIdRepository statByIdRepo = new CashierStatByIdRepositoryImpl(chaosPool);
    CashierStatByMerchantRepository statByMerchantRepo = new CashierStatByMerchantRepositoryImpl(chaosPool);
    CashierQueryRepository queryRepo = new CashierQueryRepositoryImpl(chaosPool);
    CashierCommandRepository commandRepo = new CashierCommandRepositoryImpl(chaosPool);
    ...
    CashierStatsByIdService statsByIdService = new CashierStatsByIdServiceImpl(statByIdRepo, redisService,
        tracingMetrics);
    CashierStatsByMerchant statsByMerchantService = new CashierStatsByMerchantImpl(statByMerchantRepo, redisService,
        tracingMetrics);
    CashierQueryService queryService = new CashierQueryServiceImpl(queryRepo, redisService, tracingMetrics);
    CashierCommandService commandService = new CashierCommandServiceImpl(commandRepo, queryRepo,
        merchantQueryRepository, userQueryRepository, redisService, tracingMetrics);

    // 7. Initialize Segmented Handlers
    io.example.cashier.handler.CashierQueryHandler queryHandler = new io.example.cashier.handler.CashierQueryHandler(
        statsService, statsByIdService, statsByMerchantService, queryService);
    io.example.cashier.handler.CashierCommandHandler cmdHandler = new io.example.cashier.handler.CashierCommandHandler(
        commandService);
  ...
  private Future<Void> startGrpcServer(io.example.cashier.handler.CashierQueryHandler queryHandler,
      io.example.cashier.handler.CashierCommandHandler cmdHandler, int grpcPort, ChaosManager chaosManager) {
