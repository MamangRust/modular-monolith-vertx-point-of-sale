...
public class CashierStatByIdRepositoryImpl implements CashierStatByIdRepository {
    ...
    public Future<List<CashierMonthTotalSales>> getMonthlyTotalSalesById(MonthTotalSalesCashier req) {
        Tuple args = getMonthlyTuple(req.getYear(), req.getMonth()).addInteger(req.getCashierId());
        return client
                .preparedQuery(
                        """
        ...
        return client
                .preparedQuery(
                        """
                                ...
                                """)
                .execute(Tuple.of(req.getYear(), req.getCashierId()))
                .map(this::mapCashierYearTotalSales);
    }

    @Override
    public Future<List<CashierMonthSales>> getMonthlyCashierById(MonthCashierIdRequest req) {
                        ...
                        """
                                WITH date_range AS (
                                    ...
                                    FROM cashier_activity ca ORDER BY ca.activity_month, ca.cashier_id;
                                            """)
                .execute(Tuple.of(refTs, req.getCashierId()))
                .map(this::mapCashierMonthSales);
    }

    @Override
    public Future<List<CashierYearSales>> getYearlyCashierById(YearCashierIdRequest req) {
        LocalDateTime refTs = LocalDateTime.of(req.getYear(), 1, 1, 0, 0);
        return client
                .preparedQuery(
                        """
    ...
    private Tuple getMonthlyTuple(int year, int month) {
        ...
        return Tuple.of(startDate.atStartOfDay(), endDate.atTime(LocalTime.MAX),
                startPrevDate.atStartOfDay(), endPrevDate.atTime(LocalTime.MAX));
    }
