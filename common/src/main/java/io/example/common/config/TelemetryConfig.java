...
public class TelemetryConfig {
  ...
  private boolean endpointExplicitlyDisabled;

  public TelemetryConfig(JsonObject config) {
    String envEndpoint = System.getenv("OTEL_ENDPOINT");
    this.endpointExplicitlyDisabled = (envEndpoint != null && envEndpoint.isEmpty()) || envEndpoint == null;
    if (envEndpoint != null && envEndpoint.isEmpty()) envEndpoint = null;
    this.otlpEndpoint = config.getString("otel.exporter.otlp.endpoint",
        envEndpoint != null ? "http://" + envEndpoint : null);
    this.serviceName = config.getString("service.name", "apigateway");
    this.serviceVersion = config.getString("service.version", "1.0.0");
    String envJfr = System.getenv("OTEL_JFR_ENABLED");
    this.jfrEnabled = envJfr != null ? Boolean.parseBoolean(envJfr) : config.getBoolean("otel.jfr.enabled", true);
  }

  public OpenTelemetry initialize() {
    if (endpointExplicitlyDisabled || otlpEndpoint == null || otlpEndpoint.isBlank()) {
      System.out.println("⏭️  OpenTelemetry disabled (no endpoint configured)");
