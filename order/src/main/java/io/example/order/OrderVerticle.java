...
public class OrderVerticle extends AbstractVerticle {
    ...
    public static void main(String[] args) {
        ...
        vertx.deployVerticle(new OrderVerticle(), options)
                .onSuccess(id -> {
                    ...
                    log.info("🚀 gRPC Server running on port 50055");
                })
                .onFailure(err -> {
    ...
    public void start(Promise<Void> startPromise) {
        ...
        OrderCommandRepository orderCmdRepo = new OrderCommandRepositoryImpl(chaosPool);
        OrderStatsRepository orderStatsRepo = new OrderStatsRepositoryImpl(chaosPool);
        OrderStatByMerchantRepository orderStatByMerchantRepo = new OrderStatByMerchantRepositoryImpl(chaosPool);
        ...
        final int port = tempPort > 0 ? tempPort : 50055;

        startGrpcServer(queryHandler, commandHandler, port, chaosManager)
                .onSuccess(v -> {
    ...
    private Future<Void> startGrpcServer(OrderQueryHandler queryHandler, OrderCommandHandler cmdHandler, int grpcPort, ChaosManager chaosManager) {
